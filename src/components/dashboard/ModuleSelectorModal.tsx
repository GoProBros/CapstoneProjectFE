...
import { normalizeAllowedModulesWithPreview } from '@/components/profile/modulePreviewUtils';
...
const ModuleCard: React.FC<ModuleCardProps> = ({ title, preview, onAdd, isLocked }) => {
  return (
...
export default function ModuleSelectorModal({ isOpen, onClose }: ModuleSelectorModalProps) {
  ...
  const normalizedAllowedModules = React.useMemo(() => {
    ...
    const moduleItems = normalizeAllowedModulesWithPreview(mySubscription?.allowedModules);
    return new Set(
      moduleItems
  ...
  const shouldRestrictBySubscription = mySubscription !== null && !isAdminOrStaffRole;
