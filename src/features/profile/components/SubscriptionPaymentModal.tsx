...
export function SubscriptionPaymentModal({ sub, onClose, currentSubscription }: SubscriptionPaymentModalProps) {
    ...
    const handleConfirmPayment = () => {
        const hasActiveSubscription =
            currentSubscription && isSubscriptionActive(currentSubscription.isActive);

        if (!hasActiveSubscription) {
            setConfirmState({
                isOpen: true,
                title: 'Xác nhận thanh toán',
                message: `Bạn chưa có gói thành viên đang hoạt động.
Vui lòng xác nhận để tiếp tục thanh toán và kích hoạt gói mới.`,
        ...
        const currentLevel = currentSubscription.levelOrder;
        const nextLevel = sub.levelOrder;

        if (currentLevel === nextLevel) {
            setConfirmState({
                isOpen: true,
                title: 'Xác nhận đăng ký gói',
                message: `Bạn đang có gói "${currentSubscription.subscriptionName}".\n` +
                    'Gói mới có cùng cấp độ với gói hiện tại.\n' +
                    'Thời gian sử dụng sẽ được cộng dồn khi thanh toán.',
                confirmText: 'Tiếp tục thanh toán',
                canConfirm: true,
                variant: 'info',
            });
        ...
        if (currentLevel < nextLevel) {
            setConfirmState({
                ...
                message: `Bạn đang có gói "${currentSubscription.subscriptionName}".\n` +
                    'Gói mới có cấp độ cao hơn gói hiện tại.\n' +
                    'Gói hiện tại sẽ bị hủy mặc định, không hoàn tiền và thời gian sử dụng sẽ được làm mới.',
                confirmText: 'Tiếp tục thanh toán',
                canConfirm: true,
                variant: 'warning',
            });
        ...
        setConfirmState({
            ...
            message: `Bạn đang có gói "${currentSubscription.subscriptionName}" với cấp độ cao hơn.\n` +
                'Không thể đăng ký gói thấp hơn khi gói hiện tại còn hiệu lực.',
            confirmText: 'Đã hiểu',
            canConfirm: false,
            variant: 'danger',
        });
    ...
    return (
        <>
            ...
            <ConfirmDialog
                ...
                confirmDisabled={!confirmState.canConfirm}
                onCancel={() => setConfirmState((prev) => ({ ...prev, isOpen: false }))}
                onConfirm={() => {
                    if (!confirmState.canConfirm) {
