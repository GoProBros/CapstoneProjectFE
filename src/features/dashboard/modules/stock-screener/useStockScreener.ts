...
export function useStockScreener() {
   ...
   */
  const highlightRow = useCallback((ticker: string, rowNode: any) => {
    // Move the row to position 0 so it is always visible without scrolling
    gridApiRef.current?.applyTransaction({
      remove: [rowNode.data],
      add: [rowNode.data],
      addIndex: 0,
    });
    // Toggle state so same ticker re-triggers getRowClass when searched again
    setHighlightedTicker(null);
    requestAnimationFrame(() => setHighlightedTicker(ticker));
  }, []);

  // Ticker waiting to be moved-to-top and highlighted after it appears in the grid
  const pendingHighlightTicker = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (rowsToAdd.length > 0 || rowsToUpdate.length > 0) {
      ...
      // Scroll to and highlight the pending ticker if it was just added to the grid
      if (rowsToAdd.length > 0 && pendingHighlightTicker.current) {
        const tickerToHighlight = pendingHighlightTicker.current;
        const wasAdded = rowsToAdd.some(
          (r) => r.ticker.toUpperCase() === tickerToHighlight,
        );
        if (wasAdded) {
          pendingHighlightTicker.current = null;
          requestAnimationFrame(() => {
            const rowNode = gridApi.getRowNode(tickerToHighlight)
              ?? gridApi.getRowNode(tickerToHighlight.toLowerCase());
            if (rowNode) {
              highlightRow(tickerToHighlight, rowNode);
            }
          });
        }
      }
  ...
  const handleSymbolSelect = async (ticker: string) => {
    ...
    try {
      ...
      if (isAlreadyTracked) {
        // Move row to top and apply gray highlight
        if (gridApi) {
          const rowNode = gridApi.getRowNode(upperTicker)
            ?? gridApi.getRowNode(ticker);
          if (rowNode) {
            highlightRow(upperTicker, rowNode);
          }
        }
        return;
      }

      // Mark this ticker for scroll+highlight once its row appears in the grid
      pendingHighlightTicker.current = upperTicker;
