      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-4xl font-black tracking-tight items-center flex">
            <a href={ROUTES.HOME}>
              <span className="bg-gradient-to-r from-[#4ADE80] via-emerald-300 to-blue-400 bg-clip-text text-transparent">
                KF
              </span>
    ...
    (async () => {
      try {
        ...
        if (res.data?.length) {
          ...
          const pct = (((last.close - firstClose) / firstClose) * 100).toFixed(
            1,
          );
          setDisplayPrice(last.close.toLocaleString("vi-VN"));
          setPriceChange({
            label: `${Number(pct) >= 0 ? "+" : ""}${pct}%`,
            up: Number(pct) >= 0,
          });
        }
      } catch {
        /* silently fall back to skeleton */
      }
    })();
  }, []);
  ...
  return (
    ...
    >
      <div className="relative w-full h-64 bg-gradient-to-br from-slate-900/90 to-slate-800/70 rounded-2xl border border-[#4ADE80]/30 shadow-2xl backdrop-blur-sm overflow-hidden">
        ...
        >
          <div className="text-[#4ADE80] text-base font-bold tracking-widest uppercase">
            VIB
          </div>
          <div className="text-white text-base font-bold font-mono leading-none">
            {displayPrice}
          </div>
        </motion.div>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-1 px-0.5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-white/90 text-xs font-semibold leading-none">
                    {s.label}
                  </span>
              ...
              <span className="text-[#4ADE80]/50 text-[10px] font-bold font-mono tabular-nums">
                {s.num}
              </span>
            </div>
            {/* Progress bar track */}
